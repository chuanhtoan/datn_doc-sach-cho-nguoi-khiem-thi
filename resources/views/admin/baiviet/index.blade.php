...
<div class="mdk-header-layout__content top-navbar mdk-header-layout__content--scrollable h-100">
    ...
    <div class="container-fluid">
        ...
        <h2>Bài Viết</h2>
        <p class="lead">
            Quản lý thông tin các bài viết.
        </p>
        <hr>
        <div class="card">
            <div class="py-4">
                ...
                {{-- Create Button --}}
                <div class="btn_add">
                    <a href="{{route('baiviet.create')}}" id="btn_add" name="btn_add" class="btn btn-success btn-detail">Add</a>
                </div>
                        ...
                        <tbody id="products-list" name="products-list">
                            @foreach($products as $item)
                                <tr id="product{{$item->id}}" class="active">
                                    <td>{{$item->id}}</td>
                                    <td>{{Str::limit($item->tieuDe, 30)}}</td>
                                    <td>{{App\Model\Phim::find($item->idPhim)->ten}}</td>
                                    <td>{{App\User::find($item->idUser)->username}}</td>
                                    <td>{{$item->ngay}}</td>
                                    <td>
                                        <div style="display: inline-block">
                                            <a href="{{route('baiviet.edit',$item->id)}}" class="btn btn-warning btn-detail open_modal">Edit</a>
            ...
            $(document).on('click','.delete-product',function(){
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        var tieuDeRutGon = (data.tieuDe>15)?(data.tieuDe.subString(0,15)+'...'):data.tieuDe;
                        $('#lableXoa').html('Xóa bài viết "' + tieuDeRutGon + '" ?');
